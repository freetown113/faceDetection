		...
		index += nthds_per_cta * gridDim.x)
	{
		...
		else if (code_type == CodeTypeSSD::CENTER_SIZE)
		{
			...
			else
			{
				...
				decode_bbox_center_x = p_xmin + xmin * 0.1 * p_xmax;
				decode_bbox_center_y = p_ymin + ymin * 0.1 * p_ymax;
				decode_bbox_width = p_xmax * exp(xmax * 0.2);
				decode_bbox_height = p_ymax * exp(ymax * 0.2);
			...
			case 1:
				...
				break;
			case 2:
				bbox_data[index] = decode_bbox_width + (decode_bbox_center_x - decode_bbox_width / 2.); //decode_bbox_center_x + decode_bbox_width / 2.;
				break;
			case 3:
				bbox_data[index] = decode_bbox_height + (decode_bbox_center_y - decode_bbox_height / 2.); //decode_bbox_center_y + decode_bbox_height / 2.;
				break;
			}
